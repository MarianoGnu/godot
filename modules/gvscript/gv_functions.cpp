...
const char *GVFunctions::get_func_name(Function p_func) {
...
void GVFunctions::call(Function p_func,const Variant **p_args,int p_arg_count,Variant &r_ret,Variant::CallError &r_error) {
	...
	switch(p_func) {
		...
		case INST2DICT: {
			...
			if (p_args[0]->get_type()==Variant::NIL) {
				...
				if (!obj) {
				...
				} else if (!obj->get_script_instance() || obj->get_script_instance()->get_language()!=GVScriptLanguage::get_singleton()) {
					...
					GVInstance *ins = static_cast<GVInstance*>(obj->get_script_instance());
					Ref<GVScript> base = ins->get_script();
					if (base.is_null()) {
		...
		case DICT2INST: {
			...
			for(int i=0;i<sub.get_name_count();i++) {
				...
				gvscr = gvscr->subclasses[ sub.get_name(i)];
				if (!gvscr.is_valid()) {
			...
			r_ret = gvscr->_new(NULL,0,r_error);

			GVInstance *ins = static_cast<GVInstance*>(static_cast<Object*>(r_ret)->get_script_instance());
			Ref<GVScript> gv_ref = ins->get_script();

			for(Map<StringName,GVScript::MemberInfo>::Element *E = gv_ref->member_indices.front(); E; E = E->next()) {
				if(d.has(E->key())) {
		...
		case PRINT_STACK: {
			...
			ScriptLanguage* script = GVScriptLanguage::get_singleton();
...
bool GVFunctions::is_deterministic(Function p_func) {
...
MethodInfo GVFunctions::get_info(Function p_func) {
